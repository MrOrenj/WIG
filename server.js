...
const WORD_LIST = [
    "APPLE", "BANANA", "ORANGE", "COMPUTER", "PHONE", "ELEPHANT", 
    "MOUNTAIN", "OCEAN", "FLOWER", "GUITAR", "PIZZA", "ROCKET",
    "DINOSAUR", "TREASURE", "CAMERA", "BICYCLE", "BUTTERFLY", "VOLCANO",
    "PENGUIN", "RAINBOW", "CASTLE", "DIAMOND", "TORNADO", "ISLAND"
];
...
io.on("connection", (socket) => {
    ...
    socket.on("createRoom", ({ name }) => {
        ...
        console.log(`[ROOM CREATED] Code: ${code}, Creator: ${name}, SocketID: ${socket.id}`);
    ...
    socket.on("joinRoom", ({ room, name }) => {
        ...
        console.log(`[ROOM JOINED] Room: ${room}, Player: ${name}, SocketID: ${socket.id}`);
        console.log(`[ROOM STATE] Total players in room ${room}:`, Object.values(rooms[room].players).map(p => p.name));
    ...
    socket.on("startGame", ({ room, mode }) => {
        ...
        if (playerCount < 3) {
            socket.emit("gameError", "At least 3 players are required to start the game!");
            return;
        }
    ...
    function startNewRound(room, io) {
        ...
        if (rooms[room].mode === "WIS") {
            ...
            players.forEach(id => {
                ...
                console.log(`[ROLE ASSIGNED] Player (${id}): ${role}, Word: ${playerRoles[id].word}`);
    ...
    socket.on("getGameState", ({ room }) => {
        ...
        if (socket.id in playerRoles && playerRoles[socket.id].room === room) {
            ...
            if (roomClues[room] && roomClues[room].length > 0) {
                console.log(`[SENDING STORED CLUES] Sending ${roomClues[room].length} clues to late joiner`);
            ...
            for (let socketId in rooms[room].players) {
                if (rooms[room].players[socketId].role !== "spy") {
                    expectedClues++;
    ...
    socket.on("clue", ({ room, name, clue }) => {
        ...
        console.log(`[CLUE RECEIVED] Room: ${room}, Player: ${name}, Clue: ${clue}`);
        ...
        console.log(`[CLUE MARKED] Player ${name} (${playerSocketId}) marked as given clue`);
        ...
        console.log(`[CLUE STORED] Clues stored for room ${room}:`, roomClues[room]);

        let playerCount = Object.keys(rooms[room].players).length;
        
        // Count only non-spy clues (spies don't need to give clues)
        let cluesGiven = 0;
        let expectedClues = 0;
        for (let socketId in rooms[room].players) {
            if (rooms[room].players[socketId].role !== "spy") {
        ...
        console.log(`[CLUE STATUS] Clues given: ${cluesGiven}/${expectedClues} in room ${room}`);

        // When all non-spy players have given clues, start voting phase
        if (cluesGiven >= expectedClues && expectedClues > 0) {
            console.log(`[VOTING PHASE] All clues received in room ${room}, starting voting`);
    ...
    socket.on("vote", ({ room, playerName, targetName }) => {
        ...
        console.log(`[VOTE RECEIVED] ${playerName} voted for ${targetName} in room ${room}`);
    ...
    function determineWinner(room, io) {
        ...
        console.log(`[ELIMINATION] Player ${eliminatedName} received ${voteCount[eliminatedName]} votes`);
        ...
        let result = {
            ...
            message: wasSpyEliminated ? 
                `${eliminatedName} was the SPY! Normal players WIN! 🎉` :
                `${eliminatedName} was not the spy. SPY WINS! 🕵️`,
            roundEnded: wasSpyEliminated
        };

        console.log(`[ROUND RESULT] ${result.message}`);
        io.to(room).emit("roundResult", result);

        // If spy was eliminated, game ends. Otherwise, start next round
        if (wasSpyEliminated) {
            io.to(room).emit("gameOver", result);
        } else {
